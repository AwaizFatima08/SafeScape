...
class _ProgressTabState extends ConsumerState<ProgressTab> {
  ...
  Future<void> _export() async {
    ...
    try {
      ...
      final f = File('${dir.path}/SafeScape_${safe}_${d.year}-${d.month.toString().padLeft(2, '0')}-${d.day.toString().padLeft(2, '0')}.pdf');
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      ...
      children: [
        Row(children: [
          const Expanded(child: SectionTitle('Calm & Focus Horizon')),
          SegmentedButton<int>(
        ...
        if (st.isEmpty)
          const Padding(
            ...
            child: Text('Nothing recorded yet in this period. Time spent on the Calming Canvas and Soothing Sounds, '
        ...
        Card(
          child: Column(children: [
            if (st.topModes.isEmpty)
              const ListTile(title: Text('No sessions yet', style: TextStyle(color: SC.textDim))),
            for (final (i, m) in st.topModes.take(5).indexed)
              ListTile(
                leading: CircleAvatar(backgroundColor: SC.slate3, child: Text('${i + 1}')),
                title: Text(m.label),
                trailing: Text('${(m.seconds / 60).round()} min · ${m.count}×', style: const TextStyle(color: SC.textDim)),
              ),
          ]),
        ),
        if (st.palettes.isNotEmpty || st.sounds.isNotEmpty) ...[
          const SectionTitle('Sensory preferences'),
          const Text('Average length of an undisturbed session. Longer usually means more soothing.',
        ...
        const SizedBox(height: 8),
        const Text('The PDF covers the last 30 days and opens your share menu (email, WhatsApp, Drive…). '
...
class _StatTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      FittedBox(child: Text(value, style: TextStyle(fontSize: 30, fontWeight: FontWeight.bold, color: color))),
          ...
          bottomTitles: AxisTitles(
            sideTitles: SideTitles(
              ...
              getTitlesWidget: (v, _) {
                ...
                final d = days[i].day;
                return Text(days.length > 10 ? '${d.day}/${d.month}' : names[d.weekday - 1],
