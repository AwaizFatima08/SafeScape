#!/usr/bin/env bash
#
# backup.sh — three-layer backup for Sensory SafeScape
#
#   Layer 1: local copy    -> /mnt/storage/project_backups/safescape_backup/
#   Layer 2: Google Drive  -> https://drive.google.com/drive/folders/1MQLHEGvfUOIOxbr9LbQuoM7z3pviZEFW
#   Layer 3: GitHub        -> https://github.com/AwaizFatima08/safescape (PUBLIC repo)
#
# Run:  bash /mnt/storage/projects/safescape/scripts/backup.sh
#
# Layer 2 skips .git/: Drive throttles thousands of tiny object files, and
# GitHub plus Layer 1 already hold the history.
#
# Layers 1 and 2 include .secrets/ (release keystore, key.properties) because both
...
PROJECT_DIR="/mnt/storage/projects/safescape"
LOCAL_BACKUP_ROOT="/mnt/storage/project_backups/safescape_backup"
GDRIVE_REMOTE="gdrive"
GDRIVE_FOLDER_ID="1MQLHEGvfUOIOxbr9LbQuoM7z3pviZEFW"
GIT_REMOTE_URL="git@github.com:AwaizFatima08/safescape.git"
...
( cd "$LOCAL_BACKUP_ROOT" && ls -1dt safescape_*/ 2>/dev/null | tail -n +$((KEEP_LOCAL_BACKUPS + 1)) | while read -r old; do
